...
function validateForm() {
    ...
    // ✅ Name Field - Block emails, symbols, emojis, digits
    // Allow only alphabets, spaces, hyphens (-), apostrophes (')
    const namePattern = /^[A-Za-z\s'-]+$/;
    if (!namePattern.test(name)) {
      alert("Name should not contain numbers, special characters, emojis or email addresses.");
      return false;
    }

    // Block if someone puts @ or .com etc. in name (in case regex misses it)
    const badNameKeywords = ['@', '.com', 'gmail', 'yahoo', '.in', '.org'];
    for (let keyword of badNameKeywords) {
      if (name.toLowerCase().includes(keyword)) {
        alert("Name field should not contain email-like text.");
        return false;
      }
    }
    ...
    const disposableDomains = [
      ...
      "trashmail.com", "yopmail.com", "fakeinbox.com", "getnada.com",
      "emailondeck.com", "maildrop.cc"
    ];
    ...
    // Allow only known good domains
    const allowedDomains = ["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "protonmail.com"];
    if (!allowedDomains.includes(emailDomain)) {
      alert("Not a valid email provider. Please use Gmail, Yahoo, Outlook, etc.");
      return false;
    }

    // ✅ All good → send to WhatsApp
    sendwhatsapp(name, email, subject, message);
    return false; // prevent default form submission
  }

  function sendwhatsapp(name, email, subject, message) {
    const phoneNumber = "+919426176903"; // Replace with your WhatsApp number
